...
# 12C装备可以拥有更加反則的能力，比如
# 道具，太极图定土水火风，可以给单位额外增加选择的任意守护。
# 片手，（诛仙？）圣剑无锋，三百点攻 必暴击
# 两手，盘古斧分混沌清浊 计算伤害时不按对方防御而是四围中最小的一项。大荒戟（项羽、吕布。可以当9、10级装备）
# 鞭，必定最先攻击
# 爪，拥有无视种族的魅惑
# 盾，目标绝对稳定
# 铠，巨大当巨神
...
# 先发制人，给行动值翻倍？增加一个固定值？根据自己的属性来增加？
# 或者直接改变行动顺序，往前移动一个位置，或者直到超过一个敌方单位？
# 只要从最后往前判断和移动，即使有多个单位包括双方单位有此天赋似乎也没问题
# desroyer速度慢所以适合此天赋的感觉。此外就是状态异常单位了，但状态异常单位并没有一个兵种倾向
