#! /usr/bin/env python3
# -*- coding: utf-8 -*-

# Sprite 基类
# DirtySprite(Sprite) 更多属性，感觉当真需要用Sprite的地方就需要用这个而不是基类
# Group(AbstractGroup) 基本Group
# RenderUpdates(Group) draw方法会返回改变了的区域。不太明白
# OrderedUpdates(RenderUpdates) 有序Group
# LayeredUpdates(AbstractGroup) 分层Group
# LayeredDirty(LayeredUpdates) DirtySprite专用Group
# GroupSingle(AbstractGroup) 单件？似乎可以用于鼠标停留一阵之后显示的hint

# Sprite本身是用于动画的，用在半静态上似乎有点浪费，不过想必好用

from io import BytesIO
import base64
import pygame
from pygame.locals import *
 
class Ball(pygame.sprite.Sprite):
	def __init__(self, color, initial_position):
		pygame.sprite.Sprite.__init__(self)
		# ball_file = BytesIO(base64.decodestring(
		ball_file = BytesIO(base64.b64decode(
""""""))
		self.image = pygame.image.load(ball_file)
		self.rect = self.image.fill(color, None, BLEND_ADD)
		self.rect.topleft = initial_position
 
class MoveBall(Ball):
	def __init__(self, color, initial_position, speed, border):
		super(MoveBall, self).__init__(color, initial_position)
		self.speed = speed
		self.border = border
		self.update_time = 0
 
	def update(self, current_time):
		if self.update_time < current_time:
			if self.rect.left < 0 or self.rect.left > self.border[0] - self.rect.w:
				self.speed[0] *= -1
			if self.rect.top < 0 or self.rect.top > self.border[1] - self.rect.h:
				self.speed[1] *= -1
 
			self.rect.x, self.rect.y = self.rect.x + self.speed[0], self.rect.y + self.speed[1]
			self.update_time = current_time + 10
 
pygame.init()
screen = pygame.display.set_mode([350, 350])
 
balls = pygame.sprite.Group()
for color, location, speed in [([255, 0, 0], [50, 50], [2,3]),
						([0, 255, 0], [100, 100], [3,2]),
						([0, 0, 255], [150, 150], [4,3])]:
	balls.add(MoveBall(color, location, speed, (350, 350)))
 
while True:
	if pygame.event.poll().type == QUIT: 
		break
 
	screen.fill((0,0,0,))
	current_time = pygame.time.get_ticks()
	balls.update(current_time)
	balls.draw(screen)
	# for b in balls:
	# 	b.update(current_time)
	# 	screen.blit(b.image, b.rect)
	pygame.display.update()